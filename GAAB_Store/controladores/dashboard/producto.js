  ...
  dataset.map(function (row) {
    ...
    content += `
        <tr>
            ...
            <td><a onClick="openComent(${row.id_producto})" ><img src="../../recursos/img/imagenes/file_35px.png"></a></td>
            <td><a onClick="openStock(${row.id_producto})" ><i class="material-icons">add</i></a></td>
      ...
      request.json().then(function (response) {
        ...
        if (response.status) {
          ...
          if (document.getElementById('id_estado_producto').value == 2) {
            ...
            document.getElementById('texto_im').textContent = '¿Está seguro de dar de baja este producto?'
          }
      ...
      request.json().then(function (response) {
        ...
        if (response.status) {
          ...
          if (response.dataset.estado_valoracion == 'Inapropiado') {
            document.getElementById('imagen_comentE').setAttribute('src', '../../recursos/img/imagenes/Check Circle_80px(1).png');
            document.getElementById('titulo_comentE').textContent = 'Activar'
            document.getElementById('texto_comentE').textContent = '¿Está seguro de activar este comentario?'
          } else {
            document.getElementById('imagen_comentE').setAttribute('src', '../../recursos/img/imagenes/alvertencia.png');
            document.getElementById('titulo_comentE').textContent = 'Advertencia'
            document.getElementById('texto_comentE').textContent = '¿Está seguro de dar de baja este comentario?'
          }
        } else {
          sweetAlert(2, response.exception, null);
        }
...
document.getElementById('updateComentE-form').addEventListener('submit', function (event) {
  ...
  if (document.getElementById('estado_valoracion').value == 'Inapropiado') {
    ...
    saveRow(API_PRODUCTOS, 'updateComentE', 'updateComentE-form', 'updateComentE-modal');
    M.Modal.getInstance(document.getElementById('coment-modal')).close();
    //Si es distinto
  } else {
