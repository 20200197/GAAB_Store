...
require_once('../modelos/administrar_tipo_empleado.php');
   ...
   // if (isset($_SESSION['id_empleado'])) {
        ...
        switch ($_GET['action']) {
            ...
            case 'search':
                ...
                if ($_POST['search'] == '') {
                    $result['exception'] = 'Ingrese un tipo de empleado para buscar';
                } elseif ($result['dataset'] = $tipo_empleado->searchRows($_POST['search'])) {
                    $result['status'] = 1;
                    $result['message'] = 'Tipo de empleado encontrado';
                } elseif (Database::getException()) {
                    $result['exception'] = Database::getException();
                } else {
            ...
            case 'create':
                $_POST = $tipo_empleado->validateForm($_POST);
                if (!$tipo_empleado->setTipo_empleado($_POST['tipo_empleado'])) {
                    $result['exception'] = 'Tipo  de empleado incorrecto';
                }elseif ($tipo_empleado->createRow()) {
                    $result['message'] = 'Tipo de empleado creado correctamente';
                } else {
                    $result['exception'] = Database::getException();
                }
                break;
